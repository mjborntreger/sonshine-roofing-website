...
export default function Header({
  ...
  const brandName = brandNameProp ?? siteSettings.brandName;
  const phoneDisplay = phoneDisplayProp ?? siteSettings.phone;
  const phoneHref = phoneHrefProp ?? siteSettings.phoneHref;
  ...
  const isLanding = segments.length === 0 || (segments[0] === 'locations' && segments.length === 2);
  ...
  return (
    ...
    >
      <div className={cn('hidden w-full', UTILITY_BREAKPOINT_CLASS)}></div>
      <div
        className={cn(
