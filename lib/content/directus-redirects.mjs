...
export async function getDirectusRedirects() {
  ...
  if (!response.ok) {
    throw new Error(`[directus-redirects] Directus HTTP ${response.status} ${response.statusText}`);
  }
  ...
  if (Array.isArray(json.errors) && json.errors.length) {
    throw new Error(
      json.errors
        ...
        .filter(Boolean)
        .join('; ') || '[directus-redirects] Directus request failed.',
    );
  }

  const normalized = (Array.isArray(json.data) ? json.data : []).map(toNextRule).sort(sortRules);
