...
export const PERSON_BIO_ALLOWED_TAGS = [
  'p',
  'h2',
  'h3',
  'h4',
...
const sanitizeOptions: SanitizeOptions = {
  allowedTags: [...PERSON_BIO_ALLOWED_TAGS, 'b', 'i'],
  ...
  exclusiveFilter: (frame) =>
    ['p', 'h2', 'h3', 'h4', 'ul', 'ol', 'li'].includes(frame.tag) &&
...
function appendPlainText(node: HtmlNode, output: string[]): void {
  ...
  if (['p', 'h2', 'h3', 'h4', 'ul', 'ol'].includes(node.name ?? '')) {
    output.push('\n\n');
...
export function personBioHtmlToPlainText(html: string): string {
...
export function preparePersonBioHtml(
  value: string | null | undefined,
): { html: string; text: string } | null {
  const source = value?.trim() ?? '';
  if (!source) return null;
  let html = sanitizeHtml(source, sanitizeOptions).trim();
  let text = personBioHtmlToPlainText(html);
  if (!text) return null;
  if (!/^<(?:p|h2|h3|h4|ul|ol)\b/i.test(html)) {
    html = sanitizeHtml(`<p>${html}</p>`, sanitizeOptions).trim();
    text = personBioHtmlToPlainText(html);
  }
  return { html, text };
}
