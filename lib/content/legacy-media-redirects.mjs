...
export function resolveLegacyMediaRedirect(requestUrl, rules) {
  ...
  // Capture leaves exact relative redirects in Next's native routing. Proxy
  // receives absolute destinations only and never trusts a forwarded origin.
  const target = new URL(
    rule.wildcard ? rule.destination.replaceAll('*', () => suffix) : rule.destination,
  );
