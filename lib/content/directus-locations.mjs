...
export async function fetchDirectusLocationSnapshot(projectSnapshot, env = process.env, fetcher = fetch, shared = {}) {
  ...
  data.featuredOffers = shared.offers?.filter(row => row.featured === true) ?? await read('special_offers', ['id','client.slug','status','featured','slug','title','description','discount','expiration_date','legal_disclaimer', ...fileFields('featured_image')], { ...scope, featured: { _eq: true } });
