...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM public.roofing_projects WHERE client=target_client)
    OR EXISTS (SELECT 1 FROM public.roofing_projects WHERE client=target_client AND (nullif(btrim(job_id),'') IS NULL OR zip IS NULL OR zip !~ '^[0-9]{5}(-[0-9]{4})?$')) THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.roofing_projects'::regclass AND conname='location_sonshine_project_enrichment') THEN
    EXECUTE format('ALTER TABLE public.roofing_projects ADD CONSTRAINT location_sonshine_project_enrichment CHECK (client<>%L::uuid OR (job_id IS NOT NULL AND btrim(job_id)<>'''' AND zip IS NOT NULL AND zip ~ ''^[0-9]{5}(-[0-9]{4})?$''))',target_client);
  END IF;
END;
$backfill$;
